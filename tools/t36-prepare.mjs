...
if (!fs.existsSync('build/webos/webOSTV.js') || fs.existsSync('build/service-worker.js')) throw Error('Run build:webos:packaged first');
const staging = fs.mkdtempSync(path.join(os.tmpdir(), 'stremio-t36-'));
...
for (const mode of ['hosted', 'packaged']) {
    ...
    else fs.writeFileSync(path.join(dir, 'index.html'), '<!doctype html><meta charset="utf-8"><script>location.replace(' + JSON.stringify(origin + '/?t36=storage') + ');</script>');
    fs.copyFileSync('webos/hello/hosted/icon.png', path.join(dir, 'icon.png'));
    const id = 'com.stremio.webos.t36.' + mode;
    fs.writeFileSync(path.join(dir, 'appinfo.json'), JSON.stringify({ id, version: '1.0.0', vendor: 'Stremio webOS port', type: 'web', main: 'index.html', title: 'T3.6 Updater ' + mode, resolution: '1920x1080', icon: 'icon.png' }, null, 2));
