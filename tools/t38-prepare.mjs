...
const origin = process.argv[2] || 'http://10.0.2.2:8099';
...
const staging = fs.mkdtempSync(path.join(os.tmpdir(), 'stremio-t38-'));
...
for (const mode of ['hosted', 'packaged']) {
    ...
    else fs.writeFileSync(path.join(dir, 'index.html'), '<!doctype html><meta charset="utf-8"><script>location.replace(' + JSON.stringify(origin + '/?t38=links') + ');</script>');
    fs.copyFileSync('webos/hello/hosted/icon.png', path.join(dir, 'icon.png'));
    const id = 'com.stremio.webos.t38.' + mode;
    fs.writeFileSync(path.join(dir, 'appinfo.json'), JSON.stringify({ id, version: '1.0.0', vendor: 'Stremio webOS port', type: 'web', main: 'index.html', title: 'T3.8 External links ' + mode, resolution: '1920x1080', icon: 'icon.png' }, null, 2));
