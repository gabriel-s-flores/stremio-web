// Stage and install isolated T3.9 apps; no appinfo.json is added to the repository.
...
const origin = process.argv[2] || 'http://10.0.2.2:8109';
...
if (!fs.existsSync(path.join(buildRoot, 'webos/webOSTV.js')) || fs.existsSync(path.join(buildRoot, 'service-worker.js'))) throw Error('Run t39 fixture build first');
const staging = fs.mkdtempSync(path.join(os.tmpdir(), 'stremio-t39-'));
...
for (const mode of ['hosted', 'packaged']) {
    ...
    else fs.writeFileSync(path.join(dir, 'index.html'), '<!doctype html><meta charset="utf-8"><script>location.replace(' + JSON.stringify(origin + '/?t39=clipboard') + ');</script>');
    fs.copyFileSync('webos/hello/hosted/icon.png', path.join(dir, 'icon.png'));
    const id = 'com.stremio.webos.t39.' + mode;
    fs.writeFileSync(path.join(dir, 'appinfo.json'), JSON.stringify({ id, version: '1.0.0', vendor: 'Stremio webOS port', type: 'web', main: 'index.html', title: 'T3.9 Clipboard ' + mode, resolution: '1920x1080', icon: 'icon.png' }, null, 2));
